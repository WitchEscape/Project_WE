...
public class EndingMagicCircle : MonoBehaviour
{
    ...
    public bool duringEnding = false;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (isActivated == false)
        {
            if (duringEnding)
            {
                DialogPlayer.Instance.PlayDialogSequence("ENDING_04");
                isActivated = true;
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (!duringEnding)
        {
            if (1 == playerWandCount && wasPlaying == false)
            {
                Debug.Log(2222);
                //알파값을 매 프레임 감소시킴
